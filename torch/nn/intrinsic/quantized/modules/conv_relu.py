...
class ConvTransposeReLU1d(nnq.ConvTranspose1d):

    _FLOAT_MODULE = torch.nn.intrinsic.ConvTransposeReLU1d  # type: ignore[assignment]

    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
                 padding=0, dilation=1, groups=1, bias=True,
                 padding_mode='zeros'):
        super(ConvTransposeReLU1d, self).__init__(
            in_channels, out_channels, kernel_size, stride=stride,
            padding=padding, dilation=dilation, groups=groups, bias=bias,
            padding_mode=padding_mode)
    ...
    def forward(self, input):
        ...
        if len(input.shape) != 3:
            raise ValueError("Input shape must be `(N, C, L)`!")
        return torch.relu(torch.ops.quantized.conv_transpose1d(
            input, self._packed_params, self.scale, self.zero_point))

    def _get_name(self):
        return 'QuantizedConvTransposeReLU1d'

    @classmethod
    def from_float(cls, mod):
        return super(ConvTransposeReLU1d, cls).from_float(mod)    
